...
def decode_XKCD_tuple(xkcd_values : tuple[str, ...], k : int) -> list[int]:
    ...
    for i in xkcd_values:
        new_list.append(decode_value(i))
...
def xkcd_to_list_of_weights(xkcd : str) -> list[int]:
    ...
    for i in range(len(xkcd)-1,-1,-1):
        ...
        if xkcd[i]=='0':
            ...
            List.append(int(xkcd[i])*10**counter)
            counter=0
...
def list_of_weights_to_number(weigths : list[int] ) -> int:
    ...
    for i in range(len(weigths)):
        decimal_number+=weigths[i]
        
        if weigths[i]>weigths[i-1] and i!=0:
            decimal_number-=weigths[i-1]*2 
...
if __name__ == '__main__':
    ...
    x=['111','101']
    print('10010010010100511', decode_value('10010010010100511'),'(397?)',decode_XKCD_tuple(x,1) )  